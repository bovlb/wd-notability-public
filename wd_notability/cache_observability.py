...
async def _cache_observability_snapshot() -> dict[str, Any]:
    ...
    evaluations = breakdown.get("evaluations")
    if isinstance(evaluations, dict) and evaluations.get("entries") is not None:
        total_entries = int(evaluations["entries"])
    elif breakdown.get("entries") is not None:
        total_entries = int(breakdown["entries"])
    else:
    ...
    with acquire_file_lock(CACHE_OBSERVABILITY_WORKER_LOCK_TARGET):
        while True:
            ...
            try:
                ...
                checkpoint = await CACHE.checkpoint_wal(truncate=True)
                if checkpoint is not None:
                    busy, log_frames, checkpointed_frames = checkpoint
                    print(
                        "Cache observability checkpointed WAL: "
