...
EXCLUDED_SITE_KEYS = {"apiportalwiki"}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Directory used only for the cache lock file",
    )
    parser.add_argument(
        "--concurrency",
        type=int,
    ...
    parser.add_argument(
        "--namespaces-json",
        default=str(Path(__file__).resolve().parents[3] / "wd_notability" / "data" / "namespaces_by_site.json"),
        help="Path to the old namespaces JSON cache",
    )
    parser.add_argument(
        "--site-api-urls-json",
        default=str(Path(__file__).resolve().parents[3] / "wd_notability" / "data" / "site_api_urls.json"),
...
def refresh_namespace_cache_from_json(
    ...
    lock_target = output_dir if output_dir is not None else Path("/tmp/wd-notability/namespace-cache")
    with acquire_file_lock(lock_target, "namespace"):
        ...
        cache = LookupCache(backend=create_lookup_backend())
        ...
        print("Wrote namespace cache to ToolsDB")
        print(
...
async def _fetch_json(client: httpx.AsyncClient, url: str) -> dict[str, Any]:
    if "wikidata.org/w/api.php" in url:
        response, _timings = await wikidata_session.get_with_timings(url)
        response.raise_for_status()
        return response.json()
    return await _fetch_json_with_retry(client, url)
...
async def _fetch_json_with_retry(client: httpx.AsyncClient, url: str) -> dict[str, Any]:
    ...
    *,
    ...
    with acquire_file_lock(lock_target, "namespace"):
        ...
        async with httpx.AsyncClient(timeout=60, headers=headers, follow_redirects=True) as client:
            ...
            fallback_namespaces: dict[str, dict[str, int]] = {}
            if namespaces_json is not None and namespaces_json.exists():
                try:
                    fallback_namespaces = _normalize_namespaces(_load_json(namespaces_json))
                except Exception:
                    fallback_namespaces = {}

            excluded_site_keys = [site_key for site_key in site_keys if site_key in EXCLUDED_SITE_KEYS]
            if excluded_site_keys:
                print(f"Skipping special-case site(s): {', '.join(sorted(excluded_site_keys))}")
            site_keys = [site_key for site_key in site_keys if site_key not in EXCLUDED_SITE_KEYS]
            ...
            for site_key in excluded_site_keys:
                fallback = fallback_namespaces.get(site_key)
                if fallback is not None:
                    namespaces_by_site[site_key] = fallback
            ...
            async def fetch_one(site_key: str) -> None:
                ...
                async with semaphore:
                    try:
                        ...
                        fallback = fallback_namespaces.get(site_key)
                        if fallback is not None:
                            print(f"Using cached namespaces for {site_key} after live fetch failed: {exc}")
                            namespaces_by_site[site_key] = fallback
                        else:
        ...
        used_site_keys = site_keys + [site_key for site_key in excluded_site_keys if site_key in fallback_namespaces]
        used_site_urls = {site_key: _api_url(site_urls[site_key]) for site_key in used_site_keys}
...
NAMESPACE_BUILDER = build_namespace_cache
NamespaceBuilder = build_namespace_cache

__all__ = [
    "NAMESPACE_BUILDER",
    "NamespaceBuilder",
    "build_namespace_cache",
    "refresh_namespace_cache_from_json",
]
