...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Optional lock path for serialized lookup cache writes",
    )
    parser.add_argument(
        "--qid",
        action="append",
    ...
    parser.add_argument(
        "--properties-json",
        default=str(Path(__file__).resolve().parents[3] / "wd_notability" / "data" / "property_instances_by_qid.json"),
...
def refresh_property_cache_from_json(output: Path | None, properties_json: Path) -> None:
    ...
    lock_target = output if output is not None else Path("/tmp/wd-notability/property-cache")
    with acquire_file_lock(lock_target, "property"):
        cache = LookupCache(backend=create_lookup_backend())
        cache.replace_property_instances(properties_by_qid)
        print("Wrote property cache to ToolsDB")
...
async def build_property_cache(
    ...
    *,
    ...
    lock_target = output if output is not None else Path("/tmp/wd-notability/property-cache.lock")
    with acquire_file_lock(lock_target, "property"):
        cache = LookupCache(backend=create_lookup_backend())
        cache.replace_property_instances(properties_by_qid)
        print("Wrote property cache to ToolsDB")
...
PROPERTY_BUILDER = build_property_cache
PropertyBuilder = build_property_cache

__all__ = [
    "PROPERTY_BUILDER",
    "PropertyBuilder",
    "build_property_cache",
    "refresh_property_cache_from_json",
]
