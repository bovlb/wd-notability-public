...
def build_sdc_debug_payload(context: dict[str, Any] | None) -> dict[str, Any]:
    if context is None:
        return {"context": None}
    return {
        "qid": context.get("qid"),
        "usage_count": context.get("usage_count"),
        "search_query": context.get("search_query"),
        "raw": context,
    }


def render_sdc_debug_html(report: dict[str, Any] | None) -> str:
    if report is None:
        return ""
...
SDC_DEBUG = {
    "build": build_sdc_debug_payload,
    "render": render_sdc_debug_html,
}
