...
def _normalize_replica_timestamp_filter(value: object) -> str | None:
    if value is None:
        return None
    if isinstance(value, bytes):
        try:
...
class CreationsConfig:
    ...
    def from_env(cls) -> "CreationsConfig":
        ...
        return cls(
            enabled=_env_flag("WD_NOTABILITY_CREATIONS_ENABLED", default=has_replica_env),
...
class CreationStore:
    ...
    def default_window(cls) -> tuple[str, str]:
        ...
        return (
            start.replace(microsecond=0).strftime("%Y%m%d%H%M%S"),
            end.replace(microsecond=0).strftime("%Y%m%d%H%M%S"),
        )
    ...
    def fetch_creations(
        ...
        *,
        ...
        start_ts = _normalize_replica_timestamp_filter(start)
        end_ts = _normalize_replica_timestamp_filter(end)
    ...
    def fetch_creation_metadata_for_creators(
        self,
        *,
        start: str | None = None,
        end: str | None = None,
        ...
        with closing(self._connect_replica()) as db:
            ...
            for query, query_params in ((current_sql, current_params), (archive_sql, archive_params)):
                debug_query = cursor.mogrify(query, query_params) if hasattr(cursor, "mogrify") else query
                print(f"Executing creation metadata fetch query:\n{debug_query}")
                cursor.execute(query, query_params)
