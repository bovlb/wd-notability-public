...
class IdentifiersDetector(Detector):
    # Property caches populated from Wikidata property sets on first use.
    ONLINE_ACCOUNTS_PROPERTIES: set[str] | None = None
    # Property caches populated from Wikidata property sets on first use.
    AUTHORITY_CONTROL_PROPERTIES: set[str] | None = None

    # Identifiers that are always strong N2a evidence unless they are online accounts.
    OTHER_STRONG_IDENTIFIERS = {
        "P217",  # inventory number
        "P1031",  # legal citation of this text
    }

    # Identifiers that usually count as weaker N2a evidence.
    ...
    # These property combinations count as weak N2a evidence only when they appear together.
    WEAK_COMBINATIONS = [
        (
            "P2010",  # EXIF make
            "P2009",  # EXIF model
    ...
    def _has_claim(self, entity: dict, prop: str) -> bool:
        claims = entity.get("claims", {})
        return prop in claims and bool(claims[prop])
    ...
    async def _ensure_property_sets(self) -> None:
        ...
        if self.ONLINE_ACCOUNTS_PROPERTIES is None:
            missing_qids.append("Q105388954")  # online account identifier
        if self.AUTHORITY_CONTROL_PROPERTIES is None:
            missing_qids.append("Q18614948")  # authority control

        if missing_qids:
            print(
                f"IdentifiersDetector: fetching property sets for missing QIDs: {missing_qids}")
            start_time = perf_counter()
            property_sets = await property_index.property_instances_for(missing_qids)
            if self.ONLINE_ACCOUNTS_PROPERTIES is None:
                self.__class__.ONLINE_ACCOUNTS_PROPERTIES = property_sets.get(
                    "Q105388954", set())  # online account identifier
            if self.AUTHORITY_CONTROL_PROPERTIES is None:
                self.__class__.AUTHORITY_CONTROL_PROPERTIES = property_sets.get(
                    "Q18614948", set())  # authority control
            elapsed = perf_counter() - start_time
            print(
                f"IdentifiersDetector: fetched property sets for missing QIDs in {elapsed:.2f} seconds")
    ...
    async def detect(self, entity: dict) -> AsyncGenerator[SignalResult, None]:
        ...
        for props in self.WEAK_COMBINATIONS:
            if all(self._has_claim(entity, prop) for prop in props):
                yield self.make_signal(level=NotabilityLevel.WEAK, key="identifiers_not_identifier_weak_combination", properties={"properties": props})
                continue


# Shared identifiers detector registered with the content source.
