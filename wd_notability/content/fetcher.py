...
# Batch size for pulling content contexts from the Wikidata API.
CONTENT_FETCH_CHUNK_SIZE = 50
...
class _ReplicaConfig:
    ...
    def from_env(cls) -> "_ReplicaConfig":
        defaults_file = Path(
            os.getenv(
                "WD_NOTABILITY_CONTENT_REPLICA_DEFAULTS_FILE",
        ...
        return cls(
            enabled=_env_flag(
                "WD_NOTABILITY_CONTENT_REPLICA_ENABLED", default=has_replica_env),
...
class _ContentFetcher(Source):
    ...
    def _pymysql_module():
        try:
            ...
            raise RuntimeError(
                "Content replica access requires the optional 'pymysql' Python package.") from exc
        return pymysql
    ...
    def _get_replica_connection(self):
        return self._connect_replica()

    def _reset_replica_connection(self):
        return None
    ...
    def _extract_entity(
        ...
        *,
    ...
    ) -> tuple[dict, bool] | Exception:
        entities = payload.get("entities", {}) if isinstance(
            payload, dict) else {}
        ...
        if not allow_singleton_fallback or len(entities) != 1:
            return ValueError(f"Entity {qid} not found in data")
    ...
    def _context_from_payload(
        ...
        *,
        ...
        extracted = self._extract_entity(
            qid,
            payload,
        ...
        if isinstance(claims, dict):
            has_claims_count = sum(
                len(value) for value in claims.values() if isinstance(value, list))
        has_sitelinks_count = len(sitelinks) if isinstance(
            sitelinks, dict) else 0
        return {
            "entity": entity,
    ...
    @staticmethod
    def _no_such_entity_qid(payload: object) -> str | None:
    ...
    async def _get_context_chunk(self, chunk: list[str]) -> dict[str, dict | Exception]:
        ...
        missing_qid = self._no_such_entity_qid(payload)
        if missing_qid in chunk:
            recovered_contexts: dict[str, dict | Exception] = {}
            remaining_qids = [qid for qid in chunk if qid != missing_qid]
            if remaining_qids:
                recovered_contexts.update(
                    await self._get_context_chunk(remaining_qids)
                )
            recovered_contexts[missing_qid] = EntityDeletedError(missing_qid)
            return recovered_contexts
        ...
        if self._replica_config.enabled and redirect_qids:
            try:
                ...
                redirect_timing_qid = sorted(redirect_qids)[
                    0] if redirect_qids else None
        else:
            replica_elapsed = 0.0
        ...
        for qid in chunk:
            context = self._context_from_payload(
                qid,
                payload,
                allow_singleton_fallback=len(chunk) == 1,
            )
            if isinstance(context, dict):
                if self._replica_config.enabled and context["is_redirect"]:
                    ...
                    if replica_row is None:
                        context = ValueError(
                            f"Entity {qid} not found in replica page table")
                    else:
                        replica_lastrevid, replica_redirect_target = replica_row
                        if replica_lastrevid is None:
                            context = ValueError(
                                f"Entity {qid} has no source revision on replica")
                        else:
                            entity_target = _normalize_qid(
                                context["entity"].get("id"))
                            if replica_redirect_target != entity_target:
                                ...
                                context["entity_lastrevid"] = context.get(
                                    "lastrevid")
        ...
        for qid, context in chunk_contexts.items():
            if isinstance(context, EntityDeletedError):
                continue
            if not isinstance(context, dict):
                problem_qids.append(qid)
                continue
            if context.get("lastrevid") is None and not context.get("is_deleted", False):
                problem_qids.append(qid)
        if problem_qids:
            print(
                "Content worker API fetch had problems for "
                f"{', '.join(sorted(problem_qids))}; "
                f"API URL: {request_url}"
            )
    ...
    async def get_contexts(self, qids: Collection[QID]) -> dict[QID, dict | Exception]:
        ...
        chunks = [qid_list[start: start + CONTENT_FETCH_CHUNK_SIZE]
                  for start in range(0, len(qid_list), CONTENT_FETCH_CHUNK_SIZE)]
        if not chunks:
            return contexts
    ...
    async def update_result(self, result, context: dict) -> None:
        ...
        result.has_claims_count = context["has_claims_count"]
        result.has_claims_known = True
        result.has_sitelinks_count = context["has_sitelinks_count"]
        result.content_last_revid = context.get("lastrevid")
...
# Shared content source used by the worker, API, and tests.
