...
class SitelinksDetector(Detector):
    # Project families whose sitelinks count as strong N1 evidence.
    VALID_PROJECTS = {
    ...
    # Namespaces that should never count as valid sitelinks targets.
    INVALID_NAMESPACES = {
        Namespace.TALK,
        Namespace.MEDIAWIKI,
        Namespace.SPECIAL,
    ...
    # Projects where the namespace alone is not enough to determine validity.
    ...
    # Template subpages that are known to be documentation or test surfaces.
    TEMPLATE_BAD_SUBPAGES = {"doc", "XML", "meta", "sandbox", "testcases", "TemplateData"}

    # Namespaces that are invalid for Wikidata sitelinks.
    ...
    # Namespaces that are invalid for Wiktionary sitelinks.
    WIKTIONARY_BAD_NAMESPACES = {
        Namespace.CITATIONS,
        Namespace.MAIN,
    }

    # Properties that behave like sitelinks for N1 purposes.
    SITELINK_LIKE_PROPERTIES = ("P373",)  # Commons category

    # Badges that mark a sitelink as a redirect target.
    REDIRECT_BADGES = {
        "Q70894304",  # redirect badge
        "Q70893996",  # redirect badge
    }
...
# Shared sitelinks detector registered with the content source.
