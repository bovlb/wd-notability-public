...
class SourcesDetector(Detector):
    # Cached property set populated from Wikidata on first use.
    PROPERTIES_THAT_SUGGEST_STRONG_NOTABILITY: set[str] | None = None

    # Top-level properties that are strong evidence even without references.
    OTHER_STRONG_PROPERTIES = [
        "P973",  # described at URL
        "P854",  # reference URL - should not be used at top level, but it often is
        "P1343",  # described by source
    ]

    # Top-level properties that are only weak evidence.
    WEAK_PROPERTIES = [
        "P856",  # official website
        "P1441",  # present in work
    ]

    # Reference properties that count as strong source evidence.
    ...
    # Reference properties that count as weaker source evidence.
    WEAK_REFERENCE_PROPERTIES = [
        "P248",  # stated in
        "P4656",  # Wikimedia import URL
        # NOTE: It is incorrect to use P2699 in references; P854 should be used instead.
        # But this is a very common mistake, so we still include it for completeness.
        "P2699",  # URL
    ]
    ...
    async def _ensure_property_set(self) -> None:
        if self.PROPERTIES_THAT_SUGGEST_STRONG_NOTABILITY is None:
            # Q62589316: Wikidata property for an identifier that suggests notability.
            property_sets = await property_index.property_instances_for(["Q62589316"])
            self.__class__.PROPERTIES_THAT_SUGGEST_STRONG_NOTABILITY = property_sets.get(
                "Q62589316", set())
...
# Shared sources detector registered with the content source.
