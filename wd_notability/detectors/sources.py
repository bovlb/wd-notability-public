...
# N2b policy mapping notes:
# - Strong evidence: strong reference properties (URL/archive/described-by-source)
#   and claims using properties that are instances of Q62589316.
# - Weak evidence: weaker reference/import URL signals and selected weak claim properties.
class SourcesDetector(Detector):
    ...
    # TODO: consider P953 (work available at URL). Having a copy of a work acts as a primary source for the contents of the entity, but does not directly provide access to meta information about the entity. 
    ...
    async def detect(self, entity: Entity) -> AsyncGenerator[SignalResult, None]:
            ...
            for prop in self.STRONG_REFERENCE_PROPERTIES:
                if prop in reference:
                    yield self.make_signal(
                        ...
                        key="sources_strong_reference_property",
                        properties={"property": prop, "values": self._reference_values(reference, prop)},
                    )
                    break
            else:
                for prop in self.WEAK_REFERENCE_PROPERTIES:
                    if prop in reference:
                        yield self.make_signal(
                            ...
                            key="sources_weak_reference_property",
                            properties={"property": prop, "values": self._reference_values(reference, prop)},
                        )
                        break
        ...
        for prop in strong_props:
            if self._has_property(entity, prop):
                yield self.make_signal(
                    level=NotabilityLevel.STRONG,
                    key="sources_strong_identifier",
                    properties={"property": prop, "values": self._claim_values(entity, prop)},
                )
        ...
        for prop in self.OTHER_STRONG_PROPERTIES:
            if self._has_property(entity, prop):
                yield self.make_signal(
                    level=NotabilityLevel.STRONG,
                    key="sources_other_strong_property",
                    properties={"property": prop, "values": self._claim_values(entity, prop)},
                )
        ...
        for prop in self.WEAK_PROPERTIES:
            if self._has_property(entity, prop):
                yield self.make_signal(
                    level=NotabilityLevel.WEAK,
                    key="sources_other_weak_property",
                    properties={"property": prop, "values": self._claim_values(entity, prop)},
                )
