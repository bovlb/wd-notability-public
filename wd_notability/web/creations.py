...
async def resolve_creation_bootstrap(
    qids: list[str],
) -> tuple[dict[str, EvaluationResult], dict[str, dict[str, object]]]:
    from server.report_api import _enrich_cached_results

    combined_fetch = getattr(CACHE, "get_many_with_creation_metadata", None)
    if callable(combined_fetch):
        cached_rows, metadata = await combined_fetch(qids)
    else:
        cached_rows = await CACHE.get_many(qids)
        metadata = await CACHE.get_creation_metadata_many(qids)

    cached_results = await _enrich_cached_results(cached_rows)
    if not metadata:
    ...
    return cached_results, resolved
  ...
  <head>
    ...
    <style>
      ...
      .connection-state[data-state="live"] {
        background: color-mix(in srgb, var(--strong) 16%, var(--panel));
        color: var(--strong);
      }
      ...
      .connection-state[data-state="reconnecting"] {
        background: color-mix(in srgb, var(--weak) 18%, var(--panel));
        color: var(--weak);
      }
      .connection-state[data-state="offline"] {
        background: color-mix(in srgb, var(--unknown) 12%, var(--panel));
        color: var(--unknown);
      }
      ...
      .bucket-partial-weak { color: var(--none); font-weight: 700; }
      .bucket-partial-strong { color: var(--none); font-weight: 700; }
      ...
      .bar > span.bucket-partial-weak, .swatch.bucket-partial-weak {
        background-color: var(--none);
        background-image: repeating-linear-gradient(
          135deg,
          var(--none) 0,
          var(--none) 8px,
          var(--partial-weak) 8px,
          var(--partial-weak) 16px
        );
      }
      .bar > span.bucket-partial-strong, .swatch.bucket-partial-strong {
        background-color: var(--none);
        background-image: repeating-linear-gradient(
          135deg,
          var(--none) 0,
          var(--none) 8px,
          var(--partial-strong) 8px,
          var(--partial-strong) 16px
        );
      }
    ...
    <main>
        ...
        <div class="controls-group">
          ...
          <label class="span-4">Sort by
            <select id="bucket_sort" name="bucket_sort">
              ...
              <option value="strong_rate_asc">Quality (low to high)</option>
            </select>
          </label>
      ...
      <section class="panel">
        <div class="status-row">
          ...
          <button id="connection-status" class="connection-state" data-state="offline" type="button">Offline</button>
        </div>
        <div class="meta">
