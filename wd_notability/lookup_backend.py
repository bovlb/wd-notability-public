    ...
    def load_snapshot(self) -> LookupSnapshot:
        if not self.db_path.exists():
            raise RuntimeError(
                ...
                "Run `main.py build-namespace-cache` and `main.py build-property-cache` first."
            )
    ...
    def assert_ready(self, required_property_qids: Iterable[str] = ()) -> None:
        ...
        if namespace_count == 0:
            raise RuntimeError(
                ...
                "Run `main.py build-namespace-cache` first."
            )
        if site_url_count == 0:
            raise RuntimeError(
        ...
        if wiki_subscriber_count == 0:
            raise RuntimeError(
                ...
                "Run `main.py build-wikisub-cache` first."
            )
        if missing_property_qids:
            raise RuntimeError(
                ...
                "Run `main.py build-property-cache` first."
            )
...
def create_lookup_backend(
    ...
    *,
    ...
    backend_name = os.getenv("WD_NOTABILITY_DB_BACKEND", os.getenv("WD_NOTABILITY_LOOKUP_BACKEND"))
    ...
    if backend_name in {"mariadb", "toolforge"}:
        database = os.getenv(
            "WD_NOTABILITY_LOOKUP_DATABASE",
            toolforge_database_name(default_database=default_database),
        ).strip()
        host = os.getenv("WD_NOTABILITY_DB_HOST", os.getenv("WD_NOTABILITY_LOOKUP_HOST"))
    ...
    return SqliteLookupBackend(db_path if db_path is not None else LOOKUP_CACHE_PATH)
