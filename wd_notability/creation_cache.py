                ...
                for chunk in cache._chunked(normalized):
                    ...
                    cursor = await db.execute(
                        ...
                        INSERT INTO recent_changes_cache (
                            qid, creation_time, creator_actor_id
                        )
                        VALUES {values_sql}
        ...
        if limit is None:
            cursor = await db.execute(
                ...
                LEFT JOIN recent_changes_cache rc
                  ON rc.qid = ce.qid
                WHERE rc.qid IS NULL
                   OR rc.creation_time IS NULL
                   OR rc.creator_actor_id IS NULL
...
async def list_creation_metadata(
    ...
    *,
    ...
    where_clauses = [
        "rc.creation_time IS NOT NULL",
        "rc.creator_actor_id IS NOT NULL",
    ]
    params: list[object] = []
    if start_epoch is not None:
        where_clauses.append("rc.creation_time >= ?")
    ...
    if creator_ids:
        ...
        where_clauses.append(f"rc.creator_actor_id IN ({placeholders})")
        params.extend(creator_ids)

    query = f"""
        SELECT rc.qid, rc.creator_actor_id, rc.creation_time
        FROM recent_changes_cache rc
        WHERE {' AND '.join(where_clauses)}
        ORDER BY rc.creation_time ASC, rc.qid ASC
    """
    ...
    async with cache._connect() as db:
        for start in range(0, len(qid_nums), chunk_size):
            ...
            cursor = await db.execute(
                ...
                SELECT rc.qid, rc.creator_actor_id, rc.creation_time
                FROM recent_changes_cache rc
                WHERE rc.qid IN ({placeholders})
                  AND rc.creator_actor_id IS NOT NULL
...
async def upsert_recent_changes_last_revids(cache: "EvaluationCache", qids: dict[str | int, int]) -> int:
    await cache.initialize()
    ...
    for qid, revid in qids.items():
        qid_num = cache._parse_qid(qid)
        revid_num = cache._as_uint32(revid, "recent_changes_last_revid")
    ...
    if cache._backend_name == "mariadb":
        async with cache._write_guard():
            async with cache._connect() as db:
                for chunk in cache._chunked(list(normalized.items())):
                    await db.execute("BEGIN IMMEDIATE")
                    ...
                    updated += len(rows)
                    await db.commit()
        cache._warn_slow_write("upsert_recent_changes_last_revids", started, row_count=updated)
        return updated
