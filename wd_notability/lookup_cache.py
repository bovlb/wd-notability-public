...
class LookupCache:
    def __init__(
        ...
        self._backend = backend if backend is not None else create_lookup_backend(
            db_path)
        self._loaded = False
    ...
    def _load_snapshot(self, force: bool = False) -> None:
        ...
        if not self._loaded or force:
            print("Loading lookup snapshot for namespace and property data...")
        ...
        print(
            f"Loaded lookup snapshot: {len(self._sites)} namespace site(s), "
            f"{len(self._site_api_urls)} site API URL(s), "
            f"{len(self._properties_by_qid)} property QID(s)"
        )
    ...
    def get_external_usage(self, qids: Iterable[str]) -> dict[str, dict[str, object]]:
        return self._backend.get_external_usage(qids)
    ...
    def stats(self) -> dict[str, int | str]:
        with self._lock:
            self._load_snapshot()
            namespace_prefixes = sum(len(prefixes)
                                     for prefixes in self._sites.values())
            property_instances = sum(len(properties)
                                     for properties in self._properties_by_qid.values())
            return {
                "db_path": str(getattr(self._backend, "database", "")),
