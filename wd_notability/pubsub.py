...
class PubSubStore:
    ...
    async def delete_pubsub_sessions_for_owner(self, *, owner_id: str) -> int:
        await self.cache.initialize()
        ...
        async with self.cache._write_guard():
            async with self.cache._connect() as db:
                ...
                await db.commit()
        self.cache._warn_slow_write("delete_pubsub_sessions_for_owner", started, row_count=int(cursor.rowcount))
        return int(cursor.rowcount)
    ...
    async def list_pubsub_events_for_session(
        ...
        *,
        ...
        if not session:
            raise ValueError("session_id must not be empty")
        after = self.cache._as_uint64(after_event_id, "after_event_id")
        ...
        async with self.cache._connect() as db:
            if limit is None:
                cursor = await db.execute(
                    """
                    SELECT
                        ec.last_updated AS event_id,
                        ec.last_updated AS timestamp,
                        ec.qid,
                        'summary_change' AS event_type,
                        ec.summary,
                        0 AS mask
                    FROM evaluation_cache ec
                    JOIN pubsub_sessions s
                      ON s.qid = ec.qid
                    WHERE ec.last_updated IS NOT NULL
                      AND ec.last_updated > ?
                      AND s.owner_id = ?
                      AND s.session_id = ?
                    ORDER BY ec.last_updated ASC, ec.qid ASC
                    """,
                    (after, owner, session),
                )
            else:
    ...
    async def list_pubsub_entitydata_candidates(
        self,
        limit: int | None = None,
        *,
        ...
        stale_expr = "(ec.recent_changes_last_revid IS NULL OR ec.entitydata_last_revid < ec.recent_changes_last_revid)"
        not_deleted_expr = f"(ec.qid IS NULL OR (ec.summary & {deleted_mask}) = 0)"
        interested_query = f"""
            SELECT
                s.qid AS qid,
        ...
        fallback_query = f"""
            SELECT
                ec.qid AS qid,
                0 AS subscriber_priority,
                CASE WHEN ec.entitydata_last_revid IS NULL THEN 1 ELSE 0 END AS never_evaluated,
                0 AS has_interest
            FROM evaluation_cache ec
            WHERE ec.qid != 0
              AND {not_deleted_expr}
        ...
        ctes = [f"interested AS ({interested_query})"]
        selected_parts = [
            "SELECT qid, subscriber_priority, never_evaluated, has_interest FROM interested"
        ]
        if allow_uninterested:
            ctes.append(f"fallback AS ({fallback_query})")
            selected_parts.append(
                "SELECT qid, subscriber_priority, never_evaluated, has_interest FROM fallback"
            )
    ...
    async def count_pubsub_entitydata_candidates(self, *, allow_uninterested: bool = False) -> int:
        await self.cache.initialize()
        ...
        ctes = [f"interested AS ({interested_query})"]
        selected_parts = ["SELECT qid FROM interested"]
        if allow_uninterested:
            ctes.append(f"fallback AS ({fallback_query})")
            selected_parts.append("SELECT qid FROM fallback")
    ...
    async def list_pubsub_sync_qids(
        self,
        limit: int | None = None,
        *,
        allow_uninterested: bool = False,
        ...
        ctes = [f"interested AS ({interested_query})"]
        selected_parts = ["SELECT qid, subscriber_priority, has_interest FROM interested"]
        if allow_uninterested:
            ctes.append(f"fallback AS ({fallback_query})")
            selected_parts.append("SELECT qid, subscriber_priority, has_interest FROM fallback")
        ...
        async with self.cache._connect() as db:
            if limit is None:
                cursor = await db.execute(
                    ...
                    ORDER BY has_interest DESC, subscriber_priority DESC, qid ASC
                    """
                )
            else:
                cursor = await db.execute(
    ...
    async def count_pubsub_sync_qids(self, *, allow_uninterested: bool = False) -> int:
        await self.cache.initialize()
        ...
        async with self.cache._connect() as db:
            if limit is None:
                cursor = await db.execute(
                    ...
                    """,
                    (now, n3_inlinks_mask, n3_inlinks_unknown),
                )
            else:
                cursor = await db.execute(
                    """
                    ...
                    LIMIT ?
                    """,
                    (now, n3_inlinks_mask, n3_inlinks_unknown, limit),
                )
            rows = await cursor.fetchall()
    ...
    async def list_pubsub_interest_items(self, limit: int | None = None) -> list[dict[str, Any]]:
        ...
        async with self.cache._connect() as db:
            if limit is None:
                cursor = await db.execute(
                    ...
                    WHERE qid != 0
                      AND expires_at > ?
                    GROUP BY qid, owner_id
                    ORDER BY qid ASC, owner_id ASC
                    """,
                    (now,),
                )
            else:
        ...
        for row in rows:
            ...
            wants_entitydata_rows = int(row[4]) if row[4] is not None else 0
            wants_inlinks_rows = int(row[5]) if row[5] is not None else 0
            wants_sync_rows = int(row[6]) if row[6] is not None else 0
            wants_entitydata = bool(row[7])
            wants_inlinks = bool(row[8])
            wants_sync = bool(row[9])

            item["session_rows"] += session_rows
            item["total_priority"] += total_priority
            item["owner_count"] += 1
            item["owner_ids"].append(owner_id)
            item["wants_entitydata"] = item["wants_entitydata"] or wants_entitydata
            item["wants_inlinks"] = item["wants_inlinks"] or wants_inlinks
            item["wants_sync"] = item["wants_sync"] or wants_sync
            item["workers"].append(
                {
                    "owner_id": owner_id,
                    "session_rows": session_rows,
                    "total_priority": total_priority,
                    "wants_entitydata": wants_entitydata,
                    "wants_inlinks": wants_inlinks,
                    "wants_sync": wants_sync,
                    "wants_entitydata_rows": wants_entitydata_rows,
                    "wants_inlinks_rows": wants_inlinks_rows,
                    "wants_sync_rows": wants_sync_rows,
                }
            )

        result = list(grouped.values())
        result.sort(key=lambda item: (-int(item["total_priority"]), str(item["qid"])))
        return result
