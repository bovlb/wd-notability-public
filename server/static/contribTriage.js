...
function rowHasEmptyBadgeIndication(row) {
  var badge = row.querySelector(".notability-badge svg");
  if (!badge) return false;

  var el = badge.querySelector('[data-field="has_claims"]');
  return !!el && el.getAttribute("data-value") === "false";
}
  ...
  function anyUnknownBadges() {
    ...
    for (var i = 0; i < badges.length; i++) {
      ...
      if (ignoredFields[field]) continue;
      if (window.getComputedStyle(badge).display === "none") continue;
      if (badge.getAttribute("data-value") === "unknown") {
        return true;
      }
    }
    return false;
...
function selectAllEmpty() {
  ...
  for (var i = 0; i < boxes.length; i++) {
    ...
    if (rowHasNoNotabilityBadgeIndication(row) && rowHasEmptyBadgeIndication(row)) {
      if (!cb.checked) {
        cb.checked = true;
        cb.dispatchEvent(new Event("change"));
        changed++;
  ...
  setPanelStatus("Selected " + changed + " item(s) with no-notability and empty badges.");
}
...
function selectAllRows() {
  ...
  function openDeletionDialog(qids, onSubmit) {
    ...
    optionsBox.appendChild(makeLabelCheckbox("empty item", "empty item", false));
  ...
  function makePanel() {
    ...
    var collapsedKey = "contribTriage:collapsed:" + scope;
    var collapsed = false;
    try {
      var savedCollapsed = localStorage.getItem(collapsedKey);
      collapsed = savedCollapsed === "1" || (savedCollapsed === null && window.innerWidth < 640);
    } catch (e) {}
    ...
    var collapseBtn = makeButton("Collapse", function () {
    ...
    function setCollapsed(value) {
      ...
      collapseBtn.title = collapsed ? "Expand the panel" : "Collapse the panel";
    }
    ...
    btnRow.appendChild(makeButton(
      "Select empty items",
      function () { selectAllEmpty(); },
      "Select rows whose badge says the item has no claims"
    ));
    ...
    btnRow.appendChild(makeButton(
      ...
      function () {
        ...
        if (!qids.length) {
          setPanelStatus("Nothing selected to copy.");
          return;
        }
        copyToClipboard(qids.join(" | ")).then(function () {
          setPanelStatus("Copied QIDs to clipboard.");
        });
      },
      "Copy as: Q1 | Q2 | Q3"
    ));
    ...
    btnRow.appendChild(makeButton(
      ...
      function () {
        ...
        copyToClipboard(out).then(function () {
          setPanelStatus("Copied {{Q|...}} markup to clipboard.");
        });
      },
      "Copy as: {{Q|Q1}} {{Q|Q2}} …"
    ));
    ...
    btnRow.appendChild(makeButton(
      ...
      function () {
        ...
        copyToClipboard(out).then(function () {
          setPanelStatus("Copied bulk RFD wikitext to clipboard.");
        });
      },
  ...
  function setPanelStatus(message) {
    ...
    window.clearTimeout(setPanelStatus._timer);
    if (message) {
      setPanelStatus._timer = window.setTimeout(function () {
