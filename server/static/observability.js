(function () {
  ...
  function buildCriterionSeries(fields, prefix, sourceLabel) {
    const levels = ["unknown", "none", "partial-weak", "partial-strong", "weak", "strong"];
    ...
    return Array.from(flags.entries())
      ...
      .map(([flagName, seriesByState]) => {
        const timestamps = Array.from(
          new Set(
            states.flatMap((state) => Array.from(seriesByState[state].keys()))
          )
        ).sort((left, right) => left - right);
        const currentValues = Object.fromEntries(states.map((state) => [state, 0]));
        const normalizedSeries = Object.fromEntries(
          states.map((state) => [state, []])
        );
        for (const timestamp of timestamps) {
          for (const state of states) {
            const nextValue = seriesByState[state].get(timestamp);
            if (nextValue !== undefined) {
              currentValues[state] = nextValue;
            }
            normalizedSeries[state].push([timestamp, currentValues[state]]);
          }
        }
        return {
          flagName,
  ...
  function renderStackedAreaChart(chartNode, {
    ...
    subtitle,
    seriesByLevel,
    levelOrder = ["unknown", "none", "partial-weak", "partial-strong", "weak", "strong"],
    levelLabels = {
      unknown: "unknown",
      none: "none",
      "partial-weak": "partial-weak",
    ...
    const drawOrder = [
      ...
      ...(levelOrder.includes("unknown") ? ["unknown"] : []),
    ];
    const levelColorByName = Object.fromEntries(
      levelOrder.map((level, index) => [level, palette[index % palette.length]])
    );
    const seriesEntries = drawOrder
      .map((level, index) => {
        ...
        return {
          level,
          points,
          color: levelColorByName[level] || palette[index % palette.length],
        };
      })
      .filter(Boolean);
    const resolvedLevelColors = levelColors || Object.fromEntries(
      levelOrder.map((level) => [level, colorWithAlpha(levelColorByName[level] || palette[0], 0.24)])
    );
    ...
    chart.setOption({
      ...
      tooltip: {
        ...
        formatter: (params) => {
          ...
          rows.push(`<span>Total: ${escapeHtml(formatMetricValue(total))}</span>`);
  ...
  function renderCacheBreakdownPanels(fields, container) {
    ...
    const items = [
      ...flags.map((item) => ({
        ...
        levelColors: {
          no: "rgba(15, 118, 110, 0.48)",
          yes: "rgba(217, 119, 6, 0.54)",
  ...
  function renderCacheGrowthPanel(fields, container) {
    if (!container) return;

    const ratePoints = fields && Array.isArray(fields["items.rate_per_second"]) ? fields["items.rate_per_second"] : [];
    ...
    card.innerHTML = `
      <div class="stacked-chart-head">
        <div class="title">Cache growth rate</div>
        <div class="subtitle">Items per second derived from cached item totals.</div>
      </div>
      <div class="zoom-chart"></div>
    `;
    const beforeNode = container.querySelector(".cache-breakdown-grid") || container.firstChild;
    container.insertBefore(card, beforeNode);
    renderZoomChart(card.querySelector(".zoom-chart"), "cache", "items.rate_per_second", ratePoints);
  }
  ...
  function renderWorkerSection(workerName, fields, period) {
    ...
    const workerBody = details.querySelector(".worker-body");
    ...
    if (workerName === "cache" && cacheBreakdownGrid) {
      ...
      renderCacheGrowthPanel(fields, workerBody);
